...
const RadioInput = ({ value, disabled, checked, divClassName, handleChange, labelType='label', label, name, width }: RadioInputProps) => {
  ...
  return (
    <div className={`flex items-center gap-1 ${divClassName}`}>
        ...
        {
            labelType == 'input' ? <input disabled={disabled} type='text' value={inputVal} onChange={(e) => handleInput(e.target.value)} className={`text-end px-1 w-[60px] border-lightGray border-2 ${(width ? inputVal == width : checked) ? '' : 'text-lightGray'}`} /> : <label className={checked ? '' : 'text-lightGray'}>{label}</label> 
        }
    </div>
  );
};
