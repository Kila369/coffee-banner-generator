  ...
  return (
    <div className={`flex items-center gap-1 ${divClassName}`}>
      <input
        ...
        name={name}
        value={value}
        checked={width ? inputVal == width : checked}
      ...
      {labelType == 'input' ? (
        <>
          <input
            disabled={disabled}
            type="number"
            value={parseInt(inputVal.split('p')[0])}
            onChange={e => handleInput(parseInt(e.target.value))}
            className={`text-end px-1 w-[60px] border-lightGray border-2 ${(width ? inputVal == width : checked) ? '' : 'text-lightGray'}`}
          />
          <label className={checked ? '' : 'text-lightGray'}>px</label>{' '}
        </>
      ) : (
